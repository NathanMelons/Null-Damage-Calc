...
export function checkIlluminate(gen: Generation, source: Pokemon, target: Pokemon) {
  const blocked =
  ...
  if (source.hasAbility('Illuminate') && source.abilityOn && !blocked) {
    if (target.hasAbility('Contrary')) {
      ...
      target.boosts.spa = Math.max(-6, target.boosts.atk + 2);
    } else if (target.hasAbility('Simple')) {
 ...
 * @param wonderRoomActive Whether Wonder Room is active
