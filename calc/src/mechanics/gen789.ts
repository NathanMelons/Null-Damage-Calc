...
export function calculateSMSSSV(
  ...
  checkIlluminate(gen, attacker, defender);
  checkIlluminate(gen, defender, attacker);
  ...
  const hitsPhysical = move.overrideDefensiveStat === 'def' || move.category === 'Physical' ||
    (move.named('Shell Side Arm') && getShellSideArmCategory(attacker, defender, field.isWonderRoom) === 'Physical');
  ...
  if (attacker.hasOriginalType(move.type)) {
    ...
    stabMod += 2048;
  } else if (attacker.hasAbility('Protean') && !attacker.teraType && attacker.abilityOn) {
    stabMod += 2048;
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateBPModsSMSSSV(
  ...
  if ((move.named('Facade') && attacker.hasStatus('brn', 'frb', 'par', 'psn', 'tox')) ||
...
export function calculateAttackSMSSSV(
  ...
  desc.attackEVs =
    move.named('Foul Play')
      ? getStatDescriptionText(gen, defender, attackStat, field.defenderSide.isPowerTrick, field.isWonderRoom)
      : getStatDescriptionText(gen, attacker, attackStat, field.attackerSide.isPowerTrick, field.isWonderRoom);
