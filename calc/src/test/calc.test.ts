    ...
    inGens(4, 7, ({gen, calculate, Pokemon, Move}) => {
      test(`Arceus Plate (gen ${gen})`, () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Meadow Plate Arceus Judgment vs. 31 HP / 31 SpD Blastoise: 194-230 (64.8 - 76.9%) -- guaranteed 2HKO'
        );
      });
    });
    ...
    inGens(1, 9, ({gen, calculate, Pokemon, Move, Field}) => {
      test(`Critical hits ignore attack decreases (gen ${gen})`, () => {
        ...
        if (gen < 2) {
          ...
          expect(result.desc()).toBe(
            '31 Atk burned Mew Explosion vs. 31 HP / 31 Def Vulpix on a critical hit: 729-858 (335.9 - 395.3%) -- guaranteed OHKO'
          );
        } else if (gen === 5) {
          expect(result.range()).toEqual([364, 429]);
          expect(result.desc()).toBe(
            '31 Atk burned Mew Explosion vs. 31 HP / 31 Def Vulpix on a critical hit: 364-429 (167.7 - 197.6%) -- guaranteed OHKO'
          );
        } else if (gen >= 6) {
          expect(result.range()).toEqual([273, 321]);
          expect(result.desc()).toBe(
            '31 Atk burned Mew Explosion vs. 31 HP / 31 Def Vulpix on a critical hit: 273-321 (251.1 - 295.8%) -- guaranteed OHKO'
          );
        }
    ...
    inGens(4, 9, ({gen, calculate, Pokemon, Move}) => {
      ...
      if (gen === 4) {
        ...
        test(`Iron Ball Should negate damage nullifiers (gen ${gen})`, () => {
          ...
          expect(result.desc()).toBe(
            '31 Atk Vibrava Earthquake vs. 0 HP / 0 Def Zapdos: 93-109 (28.9 - 33.9%) -- 1.2% chance to 3HKO'
          );
        });
      }
      test(`Iron Ball negates levitate (gen ${gen})`, () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Poliwrath Mud Shot vs. 0 HP / 0 SpD Mismagius: 29-35 (11.1 - 13.4%) -- possible 8HKO'
        );
      });
    });
  ...
  describe('Gen 5', () => {
    inGen(5, ({calculate, Pokemon, Move}) => {
      ...
      test('Technician with Low Kick', () => {
        ...
        expect(result.desc()).toBe(
          '31 Atk Technician Ambipom Low Kick (60 BP) vs. 252 HP / 0 Def Blissey: 272-320 (75.1 - 88.3%) -- guaranteed 2HKO'
        );
        ...
        expect(result.desc()).toBe(
          '31 Atk Ambipom Low Kick (120 BP) vs. 252 HP / 0 Def Aggron: 112-132 (63.2 - 74.5%) -- guaranteed 2HKO'
        );
      });
    });
  });
  ...
  describe('Gen 7', () => {
    inGen(7, ({calculate, Pokemon, Move, Field}) => {
      ...
      test('Loaded Field', () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Abomasnow Helping Hand Blizzard vs. 32 HP / 0 SpD Hoopa-Unbound through Light Screen with an ally\'s Friend Guard: 50-59 (16.1 - 19%)' +
      ...
      test('Wring Out', () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Technician Smeargle Wring Out (60 BP) vs. 252 HP / 0 SpD Blissey: 15-18 (4.1 - 4.9%)'
        );
      });
    ...
    describe('Gen 9', () => {
      inGen(9, ({calculate, Pokemon, Move, Field}) => {
        test('Supreme Overlord', () => {
          ...
          expect(result.desc()).toBe(
            '31 Atk Kingambit Iron Head vs. 0 HP / 0 Def Aggron: 67-79 (23.8 - 28.1%) -- 91.2% chance to 4HKO'
          );
          kingambit.alliesFainted = 5;
          result = calculate(kingambit, aggron, Move('Iron Head'));
          expect(result.range()).toEqual([100, 118]);
          expect(result.desc()).toBe(
            '31 Atk Supreme Overlord 5 allies fainted Kingambit Iron Head vs. 0 HP / 0 Def Aggron: 100-118 (35.5 - 41.9%) -- guaranteed 3HKO'
          );
