...
var DexSearch = /** @class */ (function () {
    ...
    DexSearch.prototype.getTypedSearch = function (searchType, format, speciesOrSet) {
        ...
        switch (searchType) {
            ...
            case 'encounters': return new BattleLocationSearch('location', format, speciesOrSet);
        }
        return null;
    };
...
/**
 * Preferred encounter list order (matches `name` in BattleLocationdex / encounters.js).
 * Labels from the game list are mapped to data names where they differ (e.g. tunnels, Shoal Cave).
 */
var BattleLocationdexSortOrder = [
    'Littleroot Town',
    'Littleroot Grove',
    'Route 101',
    'Oldale Town',
    'Oldale Grove',
    'Route 103',
    'Verdanturf Tunnel 1F',
    'Verdanturf Tunnel 1Bf',
    'Verdanturf Tunnel 1Bf Tiny Room',
    'Route 102',
    'Route 102 Grove',
    'Petalburg City',
    'Petalburg Grove',
...
var BattleLocationSearch = /** @class */ (function (_super) {
    ...
    BattleLocationSearch.prototype.getDefaultResults = function () {
        var results = [['header', "Location"]];
        var seen = Object.create(null);
        var byName = Object.create(null);
        for (var id in BattleLocationdex) {
            if (id === 'rates')
                continue;
            var loc = BattleLocationdex[id];
            if (!loc || typeof loc !== 'object' || !loc.name)
                continue;
            if (!byName[loc.name])
                byName[loc.name] = id;
        }
        for (var oi = 0; oi < BattleLocationdexSortOrder.length; oi++) {
            var nid = byName[BattleLocationdexSortOrder[oi]];
            if (nid && !seen[nid]) {
                results.push(['location', nid]);
                seen[nid] = true;
            }
        }
        var rest = [];
        for (var rid in BattleLocationdex) {
            if (rid === 'rates')
                continue;
            if (seen[rid])
                continue;
            var rloc = BattleLocationdex[rid];
            if (!rloc || typeof rloc !== 'object' || !rloc.name)
                continue;
            rest.push({ id: rid, name: rloc.name });
        }
        rest.sort(function (a, b) {
            return a.name.localeCompare(b.name);
        });
        for (var ri = 0; ri < rest.length; ri++) {
            results.push(['location', rest[ri].id]);
        }
        return results;
    };
