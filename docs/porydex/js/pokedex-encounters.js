/** Parse getDistribution() line: L10% 005-005 speciesid */
function parseEncounterResultLine(line) {
	if (!line || line.length < 14) return null;
	var m = line.match(/^([LWROGS])(.+?)% (\d{3})-(\d{3}) (.+)$/);
	if (!m) return null;
	var min = parseInt(m[3], 10);
	var max = parseInt(m[4], 10);
	var levelDisplay = min === max ? String(min) : min + '-' + max;
	var rateTag = m[2].replace(/z/g, '') + '%';
	return { speciesId: m[5], rateTag: rateTag, levelDisplay: levelDisplay };
}
...
var PokedexEncountersPanel = PokedexResultPanel.extend({
	...
	renderRow: function(i, offscreen) {
		...
		var parsed = parseEncounterResultLine(line);
		var id = parsed ? toID(parsed.speciesId) : '';
		var template = id ? BattlePokedex[id] : undefined;
		if (!template) {
			return '<pre>error: "' + BattleLog.escapeHTML(line) + '"</pre>';
		}
		if (offscreen) {
			return '' + (parsed ? parsed.levelDisplay + ' ' : '') + template.name + ' ' + template.abilities['0'] + ' ' + (template.abilities['1'] || '') + ' ' + (template.abilities['H'] || '') + '';
		}
		return BattleSearch.renderTaggedLocationRowInner(template, parsed.rateTag, undefined, parsed.levelDisplay);
	},
