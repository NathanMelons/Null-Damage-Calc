var PokedexPokemonPanel = PokedexResultPanel.extend({
	initialize: function(id) {
		...
		function isGmaxForm(template) {
			var f = (template && (template.forme || template.name || '') || '').toLowerCase();
			return f.indexOf('gmax') >= 0 || f.indexOf('gigantamax') >= 0;
		}
		function getCalcIconHtml(name) {
			if (typeof window.getCalcPokemonIconSrc === 'function') {
				var src = window.getCalcPokemonIconSrc(name);
				if (src) return '<img class="calc-picon pixelated" src="' + BattleLog.escapeHTML(src) + '" alt="" loading="lazy" style="margin-right:6px;vertical-align:-6px;" />';
			}
		...
		buf += '<h1>';
		if (pokemon.forme) {
			buf += '<a href="/pokemon/'+id+'" data-target="replace" class="subtle">'+pokemon.baseSpecies+'<small>-'+pokemon.forme+'</small></a>';
		} else {
			buf += '<a href="/pokemon/'+id+'" data-target="replace" class="subtle">'+pokemon.name+'</a>';
		}
		...
		var gen5Fallback = Dex.resourcePrefix + 'sprites/gen5/' + pokemon.spriteid + '.png';
		var mainSpriteSrc = (typeof getBattleSpritePngUrl === 'function' && getBattleSpritePngUrl(pokemon)) || gen5Fallback;
		buf += '<img src="' + Dex.escapeHTML(mainSpriteSrc) + '" alt="" width="96" height="96" class="sprite" onerror="this.onerror=null;this.src=\'' + gen5Fallback.replace(/\\/g, '\\\\').replace(/'/g, '\\\'') + '\'" />';

		buf += '<dl class="typeentry">';
		buf += '<dt>Types:</dt> <dd>';
		for (var i=0; i<pokemon.types.length; i++) {
			buf += '<a class="type '+toID(pokemon.types[i])+'" href="/types/'+toID(pokemon.types[i])+'" data-target="replace">'+pokemon.types[i]+'</a> ';
		}
		buf += '</dd>';
		buf += '</dl>';
		...
		buf += ''+pokemon.heightm+' m, '+pokemon.weightkg+' kg<br /><small><a class="subtle" href="/moves/grassknot" data-target="replace">Grass Knot</a>: '+gkPower+'</small>';
		buf += '</dd>';
		buf += '</dl>';
		...
		for (var i in pokemon.abilities) {
			...
			buf += '<a href="/abilities/'+toID(pokemon.abilities[i])+'" data-target="replace">'+ability+'</a>';
		...
		if (pokemon.otherFormes || pokemon.forme) {
			...
			name = getCalcIconHtml(template.name)+name;
			if (template === pokemon) {
			...
			if (otherFormes) for (var i=0; i<otherFormes.length; i++) {
				...
				if (isGmaxForm(template)) continue;
		...
		if (pokemon.cosmeticFormes) {
			...
			name = getCalcIconHtml(pokemon.name)+name;
			buf += ''+name;
		...
		if (pokemon.eggGroups) {
			buf += '<dl class="colentry"><dt>Egg groups:</dt><dd><span class="picon" style="margin-top:-12px;'+Dex.getPokemonIcon('egg')+'"></span><a href="/egggroups/'+pokemon.eggGroups.map(toID).join('+')+'" data-target="replace">'+pokemon.eggGroups.join(', ')+'</a></dd></dl>';
		...
		for (var i=0, len=moves.length; i<len; i++) {
			...
			if (move) {
				...
				buf += BattleSearch.renderTaggedMoveRow(move, desc).replace('data-target="push"', 'data-target="replace"');
			}
		}
		buf += '</ul>';
	...
	renderDetails: function() {
		var pokemon = Dex.species.get(this.id);
		var gen5Local = (typeof getBattleSpritePngUrl === 'function' && getBattleSpritePngUrl(pokemon)) || '';
		var gen5Fallback = Dex.resourcePrefix + 'sprites/gen5/' + pokemon.spriteid + '.png';
		var gen5Front = gen5Local || gen5Fallback;
		var buf = '';
		var vendorOnly = window.__DEX_VENDOR_ASSETS_ONLY === true;
		...
		if (vendorOnly) {
			if (this.id !== 'pichuspikyeared' && gen5Local) {
				buf += '<li class="resultheader"><h3>Battle sprite</h3></li>';
				buf += '<li class="content"><table class="sprites"><tr><td><img src="' + Dex.escapeHTML(gen5Local) + '" alt="" /></td></tr></table></li>';
			}
			this.$('.utilichart').html(buf);
			return;
		}
		...
		if (this.id !== 'pichuspikyeared') {
			...
			buf += '<li class="content"><table class="sprites"><tr><td><img src="' + Dex.escapeHTML(gen5Front) + '" onerror="this.onerror=null;this.src=\'' + gen5Fallback.replace(/\\/g, '\\\\').replace(/'/g, '\\\'') + '\'" /></td>';
			buf += '<td><img src="' + Dex.escapeHTML(gen5Front) + '" onerror="this.onerror=null;this.src=\'' + (Dex.resourcePrefix + 'sprites/gen5-shiny/' + pokemon.spriteid + '.png').replace(/\\/g, '\\\\').replace(/'/g, '\\\'') + '\'" /></td></table>';
