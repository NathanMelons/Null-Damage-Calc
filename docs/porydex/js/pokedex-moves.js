...
var PokedexMovePanel = PokedexResultPanel.extend({
	initialize: function(id) {
		...
		if (!move || !move.name) {
			move = (typeof BattleMovedex !== 'undefined' && BattleMovedex) ? (BattleMovedex[id] || null) : null;
		}
		if (!move || !move.name) {
			this.id = id;
			this.shortTitle = id;
			this.html(
				...
				'<p>Could not load move data for <code>' + BattleLog.escapeHTML(id) + '</code>.</p>' +
				'</div>'
			);
			return;
		}
		...
		if (move.id === 'magikarpsrevenge') {
			...
			case 'Unobtainable':
				...
				break;
			case 'CAP':
				buf += 'This is a made-up move by Smogon CAP.';
				break;
			case 'LGPE':
		...
		if (!('protect' in flags) && move.target !== 'self') {
		...
		if (!('reflectable' in flags) && move.target !== 'self' && move.category === 'Status') {
		...
		if (!('mirror' in flags) && move.target !== 'self') {
		...
		if (!('snatch' in flags) && (move.target === 'self' || move.target === 'allyTeam' || move.target === 'adjacentAllyOrSelf')) {
		...
		if (leftPanel && leftPanel.fragment.slice(0, 8) === 'pokemon/') {
			...
			if (!learnset) {
				var baseLearnsetEntry = BattleLearnsets[toID(pokemon.baseSpecies)];
				learnset = baseLearnsetEntry && baseLearnsetEntry.learnset;
			}
			var eg1 = pokemon.eggGroups[0];
			var eg2 = pokemon.eggGroups[2];
			var sources = learnset ? learnset[id] : null;
			var template = null;
			...
			while (true) {
				if (!template) {
					...
					var prevoLearnsetEntry = BattleLearnsets[template.id];
					sources = prevoLearnsetEntry && prevoLearnsetEntry.learnset ? prevoLearnsetEntry.learnset[id] : null;
				}
	...
	renderRow: function(i, offscreen) {
		...
		if (!template) {
			...
			return BattleSearch.renderTaggedPokemonRowInner(template, desc).replace('data-target="push"', 'data-target="replace"');
		}
	},
