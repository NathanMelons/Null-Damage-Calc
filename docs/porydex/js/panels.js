...
(function() {
	...
	Panels.App = Backbone.Router.extend({
		constructor: function(options) {
			...
			} else if (typeof window !== 'undefined' && window.__DEX_ROOT) {
				var dexRoot = String(window.__DEX_ROOT);
				if (dexRoot.slice(-1) !== '/') dexRoot += '/';
				this.root = dexRoot;
			}
			if ((location.search || '').substr(0, 4) === '?_ga' && window.history) {
				var gaPath = this.root;
				if (gaPath.slice(-1) === '/') gaPath = gaPath.slice(0, -1) || '/';
				history.replaceState(null, null, gaPath);
			}
			...
			// Calculator iframe sets __DEX_ROOT. Use hash routing (pathname …/index.html is not a valid route fragment).
			// Backbone loadUrl() always checks matchRoot(): deep pathnames fail against __DEX_ROOT, so History uses root '/'.
			var embedded =
				typeof window !== "undefined" &&
				window.__DEX_ROOT &&
				String(window.__DEX_ROOT).length > 0;
			var forceHash = location.protocol === "file:" || embedded;
			var usePushState = !!(window.history && window.history.pushState && !forceHash);
			var historyRoot = embedded ? "/" : this.root;
		...
		go: function(fragment, loc, replace, source, instant) {
			if (fragment) {
				...
				if (/^https?:\/\//i.test(fragment)) {
					try {
						fragment = new URL(fragment).pathname;
					} catch (e) {}
				}
				if (fragment.charAt(0) === '/') {
					var dexBase = (this.root || '').replace(/\/+$/, '');
					if (dexBase && fragment.indexOf(dexBase) === 0) {
						fragment = fragment.slice(dexBase.length).replace(/^\/+/, '');
					} else {
			...
			if (i >= 0) {
				if (i >= this.j && i <= this.i) {
					// Already visible: don't refocus/maximize. Keep split layout stable.
					this.updateURL();
					return;
				} else {
	...
	var Panel = Panels.Panel = View.extend({
		...
		handleNavigation: function(e) {
			...
			var raw = target[0];
			var linkHref = target.data('href');
			if (!linkHref && raw && raw.getAttribute) {
				var rawHref = raw.getAttribute('href');
				if (rawHref != null && rawHref !== '') linkHref = rawHref;
			}
			if (!linkHref) linkHref = target.attr('href') || target.attr('action');
			var formMethod = '';
