...
export function calculateDPP(
  ...
  const isCritical = move.isCrit && !defender.hasAbility('Battle Armor', 'Shell Armor', 'Magma Armor', 'Leaf Guard');
  ...
  if (attackBoost === 0 || (isCritical && attackBoost < 0)) {
    // attack is already rawStats value
  ...
  desc.defenseEVs =
    getStatDescriptionText(gen, defender, defenseStat, field.defenderSide.isPowerTrick, field.isWonderRoom);
  ...
  if (defenseBoost === 0 || (isCritical && defenseBoost > 0)) {
    // defense is already rawStats value
